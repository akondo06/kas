 ...
 * 
 * AKONDO: Added regex($pattern, $message) method and changed the mode "oneOf" to strict
 */
class KASValidator {
    ...
    public function oneOf($allowed, $message = null) {
        ...
        $this->setRule(__FUNCTION__, function($val, $args) {
            return in_array($val, $args[0], true);
    ...
    public function regex($pattern, $message = null) {
        $this->setRule(__FUNCTION__, function($val, $args) {
            if(!is_string($args[0]) || !is_string($val)) { return false; }
            return !(filter_var($val, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => $args[0]))) === FALSE);
    ...
    protected function _getDefaultMessage($rule, $args = null) {
        ...
        switch ($rule) {
            ...
            case 'regex':
                $message = '%s does not match the given pattern.';
                break;
